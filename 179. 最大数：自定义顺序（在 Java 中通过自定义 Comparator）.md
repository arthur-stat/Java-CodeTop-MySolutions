...
有人说这是贪心，的确有一些贪心算法的影子，因为在这里我们只定义了任意两个字符串间的序关系，考虑基于比较的排序算法，则算法最终得到的排序结果就是全局有序的。这里就蕴含了局部最优 -> 全局最优的贪心策略。

---

千万不要考虑自己手写比较逻辑甚至回溯算法，那样一定会超时的。
...
class Solution {
    public String largestNumber(int[] nums) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) != '0') return s.substring(i, s.length());
        }
        return "0";
    }
}
