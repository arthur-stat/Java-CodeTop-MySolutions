...
一个好的思路是对图结构考虑邻接表 `List<List<Integer>>`，邻接表 `adj.get(i)` 定义为所有可能的以 `s.charAt(i)` 作为左端点的回文串的**长度**的集合（不关心回文子串具体的内容，只关心其长度）。

其中，图结构的定义如下：

- 节点：字符串起始位置索引 $i$；
- 边：$i$ 至 $i+j$ 的子串，且要求该子串必须是回文子串，因为预构建了邻接表，因此不需要在 DFS 时验证边是否是回文子串而可以直接转移至下一个节点。
