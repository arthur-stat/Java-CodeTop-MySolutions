...
简单讲，我们设递归体的逻辑为：遇到数码就累积计数，直到遇到方括号，然后将方括号内的内容重复指定次。这样，递归的返回值就是解码后的字符串。每当我们碰到数码，就进行一次递归调用，然后我们暂时无需关心递归体内是如何实现的，只需要知道递归将会返回该数码对应的解码后的字符串——无论递归调用会嵌套多少层，于是在父递归中利用子递归的结果拼接解码字符串即可。
